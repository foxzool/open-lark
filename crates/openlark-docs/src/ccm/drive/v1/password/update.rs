...
mod tests {
    ...
    async fn test_update_password() {
        ...
        let request = UpdatePasswordRequest {
            ...
            new_password_hint: Some("新的密码提示".to_string()),
            department_passwords: Some(vec![
                DepartmentPassword {
                    department_id: "dept_001".to_string(),
                    password: "new_dept_password".to_string(),
                    password_hint: Some("新的部门密码提示".to_string()),
                },
            ]),
        };

        let result = update_password(request, &config, None).await;
        assert!(result.is_ok());
    }
}