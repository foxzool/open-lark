...
pub async fn get_folder_meta(
    ...
    let url = format!(
        ...
        config.base_url, request.folder_token
    );
...
mod tests {
    ...
    async fn test_get_folder_meta() {
        ...
        let request = GetFolderMetaRequest {
            folder_token: "test_folder_token".to_string(),
        };

        let result = get_folder_meta(request, &config, None).await;
        assert!(result.is_ok());
    }
}