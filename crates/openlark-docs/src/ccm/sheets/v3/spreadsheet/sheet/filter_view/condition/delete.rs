...
//! 删除筛选视图的筛选范围某一列的筛选条件
//!
//! 文档链接: https://open.feishu.cn/document/server-docs/docs/sheets-v3/spreadsheet-sheet-filter_view/condition/delete
...
/// # 示例
/// ```rust
/// let response = delete_filter_condition(&config, "sheet_token_123", "sheet_id_456", "fv_abc123", "A")
///     .await?;
/// if response.success {
///     println!("筛选条件删除成功");
/// }
...
pub async fn delete_filter_condition(
    ...
    let api_response: SheetsResponse<DeleteFilterConditionResponse> =
...
/// 删除筛选条件构建器
pub struct DeleteFilterConditionBuilder<'a> {
    config: &'a Config,
...
impl<'a> DeleteFilterConditionBuilder<'a> {
    /// 创建新的构建器实例
    ...
    /// 执行删除筛选条件请求
    pub async fn execute(self) -> SDKResult<DeleteFilterConditionResponse> {
        ...
        delete_filter_condition(self.config, &spreadsheet_token, &sheet_id, &filter_view_id, &column_id).await
    }
}
...
/// # 返回
/// 返回删除筛选条件的构建器
pub fn delete_filter_condition_builder(config: &Config) -> DeleteFilterConditionBuilder {
    DeleteFilterConditionBuilder::new(config)
}
...
mod tests {
    ...
    #[test]
    fn test_delete_filter_condition_builder() {
        let config = Config::default();
        let builder = delete_filter_condition_builder(&config)
