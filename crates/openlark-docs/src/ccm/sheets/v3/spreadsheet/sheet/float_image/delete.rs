...
/// 删除浮动图片构建器
pub struct DeleteFloatImageBuilder<'a> {
    config: &'a Config,
...
impl<'a> DeleteFloatImageBuilder<'a> {
    /// 创建新的构建器实例
    ...
    /// 执行删除浮动图片请求
    pub async fn execute(self) -> SDKResult<DeleteFloatImageResponse> {
        ...
        let api_response: SheetsResponse<DeleteFloatImageResponse> =
...
/// # 示例
/// ```rust
/// let response = delete_float_image(&config, "sheet_token_123", "sheet_id_456", "img_abc123")
///     .await?;
/// if response.success {
///     println!("浮动图片删除成功");
/// }
...
/// # 返回
/// 返回删除浮动图片的构建器
pub fn delete_float_image_builder(config: &Config) -> DeleteFloatImageBuilder {
    DeleteFloatImageBuilder::new(config)
}
...
mod tests {
    ...
    #[test]
    fn test_delete_float_image_builder() {
        let config = Config::default();
        let builder = delete_float_image_builder(&config)
