...
//!
//! 文档链接: https://open.feishu.cn/document/server-docs/docs/sheets-v3/spreadsheet-sheet-filter_view/condition/update
...
/// 更新筛选条件构建器
pub struct UpdateFilterConditionBuilder<'a> {
    config: &'a Config,
...
impl<'a> UpdateFilterConditionBuilder<'a> {
    /// 创建新的构建器实例
    ...
    /// 执行更新筛选条件请求
    pub async fn execute(self) -> SDKResult<UpdateFilterConditionResponse> {
        ...
        let url = format!(
            ...
            self.config.base_url, spreadsheet_token, sheet_id, filter_view_id, column_id
        );
        ...
        let api_response: SheetsResponse<UpdateFilterConditionResponse> =
...
/// # 示例
/// ```rust
/// let response = update_filter_condition(&config, "sheet_token_123", "sheet_id_456", "fv_abc123", "A")
///     .operator("equals")
...
///     .await?;
/// ```
pub fn update_filter_condition(
    config: &Config,
    spreadsheet_token: &str,
    sheet_id: &str,
    filter_view_id: &str,
    column_id: &str,
) -> UpdateFilterConditionBuilder {
    UpdateFilterConditionBuilder::new(config)
...
mod tests {
    ...
    #[test]
    fn test_update_filter_condition_builder() {
        let config = Config::default();
        let builder = update_filter_condition(&config, "sheet_token_123", "sheet_id_456", "fv_abc123", "A")
            .operator("equals")
        ...
        assert_eq!(builder.request.operator, "equals");
        assert!(builder.request.value.is_some());
        assert_eq!(builder.request.ignore_case, Some(false));
    }
}