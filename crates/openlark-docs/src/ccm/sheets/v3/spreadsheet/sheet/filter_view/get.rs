...
//!
//! 文档链接: https://open.feishu.cn/document/server-docs/docs/sheets-v3/spreadsheet-sheet-filter_view/get
...
use crate::ccm::sheets::v3::models::{
    FilterViewInfo, SpreadsheetToken, SheetId, FilterViewId, SheetsResponse
};
...
/// # 示例
/// ```rust
/// let response = get_filter_view(&config, "sheet_token_123", "sheet_id_456", "fv_abc123")
///     .await?;
/// println!("筛选视图名称: {}", response.filter_view.name);
/// println!("筛选范围: {}", response.filter_view.range);
/// ```
pub async fn get_filter_view(
    ...
    let api_response: SheetsResponse<GetFilterViewResponse> =
...
/// 获取筛选视图构建器
pub struct GetFilterViewBuilder<'a> {
    config: &'a Config,
...
impl<'a> GetFilterViewBuilder<'a> {
    ...
    /// 执行获取筛选视图请求
    pub async fn execute(self) -> SDKResult<GetFilterViewResponse> {
        ...
        get_filter_view(self.config, &spreadsheet_token, &sheet_id, &filter_view_id).await
    }
}
...
/// # 返回
/// 返回获取筛选视图的构建器
pub fn get_filter_view_builder(config: &Config) -> GetFilterViewBuilder {
    GetFilterViewBuilder::new(config)
}
...
mod tests {
    ...
    #[test]
    fn test_get_filter_view_builder() {
        let config = Config::default();
        let builder = get_filter_view_builder(&config)
