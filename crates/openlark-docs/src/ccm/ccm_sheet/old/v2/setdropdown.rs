...
/// docPath: https://open.feishu.cn/document/server-docs/docs/sheets-v3/data-validation/create-data-validation
use openlark_core::{
...
pub struct SetDropdownParams {
    /// 电子表格的 token
    #[serde(rename = "spreadsheetToken")]
    pub spreadsheet_token: String,
    /// 数据验证设置
    #[serde(rename = "dataValidationSettings")]
    pub data_validation_settings: DataValidationSettings,
}
...
pub struct DataValidationSettings {
    /// 数据验证规则
    #[serde(rename = "dataValidationRules")]
...
/// 创建数据验证规则响应
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SetDropdownResponse {
    /// 创建结果
    pub data: Option<SetDropdownResult>,
}

/// 创建数据验证规则结果
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SetDropdownResult {
    /// 电子表格的 token
    #[serde(rename = "spreadsheetToken")]
    pub spreadsheet_token: String,
    /// 创建的数据验证规则列表
...
impl ApiResponseTrait for SetDropdownResponse {
...
/// 创建数据验证规则请求
pub struct SetDropdownRequest {
    config: Config,
}

impl SetDropdownRequest {
    /// 创建数据验证规则请求
    ...
    pub async fn execute(self, params: SetDropdownParams) -> SDKResult<SetDropdownResponse> {
        ...
        let api_request: ApiRequest<SetDropdownResponse> = ApiRequest::post(
            &api_endpoint.to_url(),
        )
