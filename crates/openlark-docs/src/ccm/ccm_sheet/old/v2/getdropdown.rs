...
/// 获取数据验证规则响应
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GetDropdownResponse {
    /// 获取结果
    pub data: Option<GetDropdownResult>,
}

/// 获取数据验证规则结果
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GetDropdownResult {
    /// 电子表格的 token
    #[serde(rename = "spreadsheetToken")]
    pub spreadsheet_token: String,
    /// 数据验证规则列表
...
impl ApiResponseTrait for GetDropdownResponse {
...
/// 获取数据验证规则请求
pub struct GetDropdownRequest {
    config: Config,
}

impl GetDropdownRequest {
    /// 创建获取数据验证规则请求
    ...
    pub async fn execute(
        ...
        let mut api_request: ApiRequest<GetDropdownResponse> =
