...
/// docPath: https://open.feishu.cn/document/server-docs/docs/sheets-v3/conditional-formatting/batch-update-conditional-formats
use openlark_core::{
...
impl UpdateConditionFormatRequest {
    ...
    pub async fn execute(
        ...
        let api_endpoint =
            CcmSheetApiOld::ConditionFormatsBatchUpdate(params.spreadsheet_token.clone());

        // 创建API请求 - 使用类型安全的URL生成
        let mut api_request: ApiRequest<UpdateConditionFormatResponse> = ApiRequest::post(
            &api_endpoint.to_url(),
        )
