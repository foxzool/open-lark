...
impl AppRoleService {
    ...
    pub async fn create_role(&self, request: &CreateRoleRequest) -> SDKResult<CreateRoleResponse> {
        ...
        body.insert("role_name".to_string(), serde_json::to_value(&request.role_name)?);
        ...
        let api_req = ApiRequest {
            ...
            api_path: format!("/open-apis/bitable/v1/apps/{}/roles", request.app_token),
    ...
    pub async fn update_role(&self, request: &UpdateRoleRequest) -> SDKResult<UpdateRoleResponse> {
        ...
        if let Some(ref role_name) = request.role_name {
            body.insert("role_name".to_string(), serde_json::to_value(role_name)?);
        }
        if let Some(ref description) = request.description {
            body.insert("description".to_string(), serde_json::to_value(description)?);
        }
