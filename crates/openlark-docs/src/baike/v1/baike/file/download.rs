...
impl<'a> DownloadFileBuilder<'a> {
    ...
    pub async fn execute(self) -> SDKResult<DownloadFileResponse> {
        let path = format!("/open-apis/baike/v1/files/{}/download", self.file_token);
        ...
        if response.status().is_success() {
            ...
            let text = response.text().await.map_err(|e| openlark_core::error::CoreError::network_msg(e.to_string()))?;
            Err(openlark_core::error::CoreError::api_error(
