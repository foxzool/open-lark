...
pub struct SearchSpaceResponse {
...
impl ApiResponseTrait for SearchSpaceResponse {
...
pub async fn search_space(
    ...
    let api_endpoint = BaikeApiV1::SearchSpace;

    let api_request: ApiRequest<SearchSpaceResponse> =
    ...
    let resp: SearchSpaceResponse = response.data.ok_or_else(|| {
