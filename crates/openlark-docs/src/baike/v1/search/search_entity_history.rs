...
pub struct SearchResponseResponse {
...
impl ApiResponseTrait for SearchResponseResponse {
...
/// 在知识库中搜索词条。
/// docPath: https://open.feishu.cn/document/baike-v1/search/entity
pub async fn search_entity_history(
    config: &Config,
    query: &str,
    space_id: Option<&str>,
    ...
    let api_endpoint = BaikeApiV1::SearchResponse;

    let api_request: ApiRequest<SearchResponseResponse> =
    ...
    let resp: SearchResponseResponse = response.data.ok_or_else(|| {
