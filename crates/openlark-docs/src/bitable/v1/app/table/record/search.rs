...
pub struct SearchRecordRequest {
    /// 配置信息
    config: Config,
    api_request: ApiRequest<SearchRecordResponse>,
    /// 多维表格的 app_token
    app_token: String,
    /// 数据表的 table_id
    table_id: String,
...
impl SearchRecordRequest {
    ...
    pub async fn execute(self) -> SDKResult<SearchRecordResponse> {
        ...
        // 构建请求体
        let request_body = SearchRecordRequestBody {
        ...
        // 解析响应数据
        let search_data: SearchRecordData = response
            .data
            .and_then(|data| serde_json::from_value(data).ok())
            .ok_or_else(|| validation_error("解析搜索记录响应失败", "响应数据格式不正确"))?;

        Ok(SearchRecordResponse {
            items: search_data.items,
            has_more: search_data.has_more,
            page_token: search_data.page_token,
