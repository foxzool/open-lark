...
impl AppAccessTokenService {
    ...
    /// 自建应用获取应用访问令牌
    pub async fn internal(&self) -> SDKResult<crate::auth::token::AccessToken> {
        let request = GetTokenRequest::self_build_app_access_token();
        self.token_client.get_access_token(request).await
    }

    /// 商店应用获取应用访问令牌
    pub async fn store(&self) -> SDKResult<crate::auth::token::AccessToken> {
        let request = GetTokenRequest::store_app_access_token();
        self.token_client.get_access_token(request).await
    }

    /// 重新推送应用票据
    pub async fn ticket_resend(&self, app_id: &str) -> SDKResult<TicketResendResponse> {
        // TODO: 实现应用票据重新推送逻辑
...
impl TenantAccessTokenService {
    ...
    /// 自建应用获取租户访问令牌
    pub async fn internal(&self, tenant_key: &str) -> SDKResult<crate::auth::token::AccessToken> {
        let request = GetTokenRequest::self_build_tenant_access_token(tenant_key.to_string());
        self.token_client.get_access_token(request).await
    }

    /// 商店应用获取租户访问令牌
    pub async fn store(&self, tenant_key: &str) -> SDKResult<crate::auth::token::AccessToken> {
...
impl AppPermissionsService {
    ...
    /// 查询应用权限
    pub async fn query(&self, app_id: &str) -> SDKResult<AppPermissionsResponse> {
        // TODO: 实现应用权限查询逻辑
