...
impl CacheManager {
    ...
    /// 获取缓存的令牌
    pub async fn get_token(&self, key: &str) -> SDKResult<Option<TokenInfo>> {
        debug!("Getting token from cache: {}", key);
        Ok(self.cache.get(key).await)
    }

    /// 缓存令牌
    pub async fn put_token(&self, key: &str, token: TokenInfo) -> SDKResult<()> {
    ...
    /// 移除缓存的令牌
    pub async fn remove_token(&self, key: &str) -> SDKResult<()> {
    ...
    /// 清空所有缓存
    pub async fn clear_cache(&self) -> SDKResult<()> {
    ...
    /// 批量获取令牌
    pub async fn batch_get_tokens(&self, keys: &[String]) -> Vec<SDKResult<Option<TokenInfo>>> {
    ...
    /// 批量缓存令牌
    pub async fn batch_put_tokens(&self, token_pairs: Vec<(String, TokenInfo)>) -> SDKResult<()> {
    ...
    /// 预热缓存
    pub async fn warmup_cache(&self, tokens: Vec<(String, TokenInfo)>) -> SDKResult<()> {
    ...
    /// 根据模式删除缓存项
    pub async fn remove_cache_by_pattern(&self, pattern: &str) -> SDKResult<usize> {
    ...
    /// 更新缓存配置
    pub async fn update_config(&mut self, config: CacheConfig) -> SDKResult<()> {
