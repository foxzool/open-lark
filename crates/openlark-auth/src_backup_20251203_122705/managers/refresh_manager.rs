...
impl RefreshManager {
    ...
    /// 智能刷新（检查是否需要刷新）
    pub async fn smart_refresh_token(&self, key: &str) -> SDKResult<Option<TokenInfo>> {
    ...
    /// 检查令牌是否需要刷新
    pub async fn should_refresh_token(&self, key: &str) -> SDKResult<bool> {
    ...
    /// 定时刷新令牌（后台任务）
    pub async fn scheduled_refresh(&self) -> SDKResult<RefreshResult> {
    ...
    /// 强制刷新所有令牌
    pub async fn force_refresh_all(&self) -> SDKResult<RefreshResult> {
