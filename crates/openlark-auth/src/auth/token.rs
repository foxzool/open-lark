...
//! # 错误处理
//!
//! 令牌相关错误通过 `TokenValidationResult` 和 `AuthResult` 处理：
//!
//! ```rust
...
impl TokenInfo {
    ...
    /// 转换为AccessToken（用于API响应）
    pub fn to_access_token(&self) -> AccessToken {
        AccessToken {
            access_token: self.access_token.clone(),
            token_type: self.token_type.as_str().to_string(),
    ...
    /// 从AccessToken创建TokenInfo
    pub fn from_access_token(token: AccessToken, app_type: String, token_type: TokenType) -> Self {
        let now = SystemTime::now();
...
mod tests {
    ...
    #[test]
    fn test_access_token_conversion() {
        ...
        let access_token = token_info.to_access_token();
        assert_eq!(access_token.access_token, "test_access_token");
        assert_eq!(access_token.token_type, "app_access_token");
    ...
    fn test_from_access_token() {
        let access_token = AccessToken {
            ...
            token_type: "app_access_token".to_string(),
            expires_in: 3600,
            refresh_token: Some("refresh_token".to_string()),
            scope: Some("test_scope".to_string()),
        };

        let token_info = TokenInfo::from_access_token(
            access_token,
            "test_app".to_string(),
            TokenType::AppAccessToken,
        );
        ...
        assert_eq!(token_info.app_type, "test_app");
