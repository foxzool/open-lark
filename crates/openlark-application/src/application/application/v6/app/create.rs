...
impl CreateAppRequest {
    ...
    pub async fn execute_with_options(
        ...
        let path = "/open-apis/application/application/v1/app/create"
            .replace("application", "application")
            .replace("security", "acs")
            .replace("personal_settings", "personal_settings");
        let req: ApiRequest<CreateAppResponse> = ApiRequest::get(&path);

        let resp = Transport::request(req, &self.config, Some(option)).await?;
        resp.data.ok_or_else(|| {
            openlark_core::error::validation_error("app create", "响应数据为空")
        })
    }
}
