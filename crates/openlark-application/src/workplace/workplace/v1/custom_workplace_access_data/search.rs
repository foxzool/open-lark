...
impl AccessDataSearchCustomBuilder {
    ...
    /// 执行请求
    pub async fn execute(self) -> SDKResult<AccessDataSearchCustomResponse> {
    ...
    ) -> SDKResult<AccessDataSearchCustomResponse> {
        let url = "/open-apis/workplace/v1/custom_workplace_access_data/search";

        let request = AccessDataSearchCustomRequest {
...
struct AccessDataSearchCustomRequest {
    /// 开始日期 (YYYY-MM-DD)
...
/// 定制工作台访问数据查询响应
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct AccessDataSearchCustomResponse {
    /// 访问数据列表
