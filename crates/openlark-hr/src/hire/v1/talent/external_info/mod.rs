...
impl TalentExternalInfo {
    ...
    pub async fn post_open_apis_hire_v1_talents_by_talent_id_external_info(&self, talent_id: impl AsRef<str>, payload: Option<&Value>) -> SDKResult<Value> {
    ...
    pub async fn put_open_apis_hire_v1_talents_by_talent_id_external_info(&self, talent_id: impl AsRef<str>, payload: Option<&Value>) -> SDKResult<Value> {
        let mut path = "/open-apis/hire/v1/talents/:talent_id/external_info".to_string();
        path = path.replace(":talent_id", talent_id.as_ref());
