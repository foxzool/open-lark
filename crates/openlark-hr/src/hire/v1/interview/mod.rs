...
impl Interview {
    ...
    pub async fn get_open_apis_hire_v1_interviews(
        &self,
        payload: Option<&Value>,
    ) -> SDKResult<Value> {
        let path = "/open-apis/hire/v1/interviews".to_string();
    ...
    pub async fn get_open_apis_hire_v1_interviews_get_by_talent(
        &self,
        payload: Option<&Value>,
    ) -> SDKResult<Value> {
        let path = "/open-apis/hire/v1/interviews/get_by_talent".to_string();
        let method = Method::GET;
        let (query, body) = match method {
            Method::GET | Method::DELETE => (payload, None),
            _ => (None, payload),
