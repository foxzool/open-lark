...
impl UserTaskRemedy {
    ...
    pub async fn post_open_apis_attendance_v1_user_task_remedys(&self, payload: Option<&Value>) -> SDKResult<Value> {
    ...
    pub async fn post_open_apis_attendance_v1_user_task_remedys_query_user_allowed_remedys(&self, payload: Option<&Value>) -> SDKResult<Value> {
        let mut path = "/open-apis/attendance/v1/user_task_remedys/query_user_allowed_remedys".to_string();
        let method = Method::POST;
        let (query, body) = match method {
            Method::GET | Method::DELETE => (payload, None),
            _ => (None, payload),
    ...
    pub async fn post_open_apis_attendance_v1_user_task_remedys_query(&self, payload: Option<&Value>) -> SDKResult<Value> {
