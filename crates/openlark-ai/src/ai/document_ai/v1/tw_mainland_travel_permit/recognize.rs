...
pub struct TwMainlandTravelPermitRecognizeBody {
...
/// 台湾通行证识别响应
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TwMainlandTravelPermitRecognizeResponse {
    /// 识别结果
    #[serde(skip_serializing_if = "Option::is_none")]
    pub data: Option<TwMainlandTravelPermitRecognizeResult>,
}

impl openlark_core::api::ApiResponseTrait for TwMainlandTravelPermitRecognizeResponse {}

/// 台湾通行证识别结果
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TwMainlandTravelPermitRecognizeResult {
    /// 姓名
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// 台湾通行证识别请求
#[derive(Debug, Clone)]
pub struct TwMainlandTravelPermitRecognizeRequest {
    config: Config,
}

impl TwMainlandTravelPermitRecognizeRequest {
    /// 创建新的台湾通行证识别请求
...
/// 台湾通行证识别请求构建器
#[derive(Debug, Clone)]
pub struct TwMainlandTravelPermitRecognizeRequestBuilder {
    request: TwMainlandTravelPermitRecognizeRequest,
    file_token: Option<String>,
    is_async: Option<bool>,
}

impl TwMainlandTravelPermitRecognizeRequestBuilder {
    /// 创建新的构建器
    pub fn new(config: Config) -> Self {
        Self {
            request: TwMainlandTravelPermitRecognizeRequest::new(config),
            file_token: None,
            is_async: None,
        }
    }
    ...
    /// 构建请求体
    pub fn body(self) -> TwMainlandTravelPermitRecognizeBody {
        TwMainlandTravelPermitRecognizeBody {
    ...
    /// 执行请求
    pub async fn execute(self) -> SDKResult<TwMainlandTravelPermitRecognizeResponse> {
...
pub async fn tw_mainland_travel_permit_recognize(
    ...
    tw_mainland_travel_permit_recognize_with_options(config, body, RequestOption::default()).await
}

/// 执行台湾通行证识别（支持自定义选项）
pub async fn tw_mainland_travel_permit_recognize_with_options(
    config: &Config,
    body: TwMainlandTravelPermitRecognizeBody,
    option: RequestOption,
) -> SDKResult<TwMainlandTravelPermitRecognizeResponse> {
...
mod tests {
    ...
    fn test_builder_default_state() {
        let config = Config::builder()
            ...
            .build();
        let builder = TwMainlandTravelPermitRecognizeRequestBuilder::new(config.clone());
    ...
    fn test_body_validation_empty_file_token() {
        let body = TwMainlandTravelPermitRecognizeBody {
