...
mod tests {
    ...
    async fn test_req_translator_with_different_token_types() {
        let mut api_req =
            ApiRequest::<serde_json::Value>::post("https://open.feishu.cn/open-apis/test").body(
        ...
        for method in methods.iter() {
            let mut api_req = match *method {
                ...
                reqwest::Method::DELETE => {
                    ApiRequest::<()>::delete("https://open.feishu.cn/test/path")
                }
    ...
    async fn test_req_translator_with_marketplace_app() {
        let mut api_req =
            ApiRequest::<()>::get("https://open.feishu.cn/open-apis/marketplace/test");
