...
mod tests {
    ...
    #[test]
    fn test_defaults() {
        let policy = defaults::default_retry_policy();
        assert!(policy.is_retryable());
        assert_eq!(policy.max_retries(), 3);

        let context = defaults::default_error_context();
    ...
    /// 测试 analyze_error 对不同类型错误
    #[test]
    fn test_analyze_error_various_types() {
        // 网络错误
        let network = network_error("连接失败");
        let analysis = analyze_error(&network);
        assert_eq!(analysis.error_type, ErrorType::Network);
        assert_eq!(analysis.recovery_action, "retry");
        assert!(analysis.is_retryable);

        // 认证错误
        let auth = authentication_error("认证失败");
        let analysis = analyze_error(&auth);
        assert_eq!(analysis.error_type, ErrorType::Authentication);
        assert_eq!(analysis.recovery_action, "reauthenticate");
        assert!(!analysis.is_retryable);

        // 验证错误
        let validation = validation_error("field", "无效");
        let analysis = analyze_error(&validation);
        assert_eq!(analysis.error_type, ErrorType::Validation);
        assert_eq!(analysis.recovery_action, "fix_input");

        // 配置错误
        let config = configuration_error("配置缺失");
        let analysis = analyze_error(&config);
        assert_eq!(analysis.error_type, ErrorType::Configuration);
        assert_eq!(analysis.recovery_action, "fix_config");

        // 限流错误
        let rate_limit = rate_limit_error(100, Duration::from_secs(60), None);
        let analysis = analyze_error(&rate_limit);
        assert_eq!(analysis.error_type, ErrorType::RateLimit);
        assert_eq!(analysis.recovery_action, "slowdown");
    }

    /// 测试 analyze_error 返回的完整字段
    #[test]
    fn test_analyze_error_complete_fields() {
        let mut ctx = ErrorContext::new();
        ctx.set_request_id("analyze-test-001");
        ctx.set_operation("test_operation");
        ctx.set_component("test_component");

        // 使用网络错误测试系统错误判定
        let error = network_error_with_details(
            "连接失败",
            Some("/api/test".to_string()),
            Some("analyze-test-001".to_string()),
        );
        ...
        assert!(analysis.system_error);
        assert_eq!(analysis.recovery_action, "retry");
        assert!(analysis.context_summary.contains("analyze-test-001"));
    }

    /// 测试 summarize_context 函数
    #[test]
    fn test_summarize_context_various_states() {
        // 带 request_id 的上下文
        let mut ctx_with_req = ErrorContext::new();
        ctx_with_req.set_request_id("req-abc");
        let error = CoreError::Api(Box::new(ApiError {
            status: 500,
            endpoint: "/api".into(),
            message: "error".to_string(),
            source: None,
            code: ErrorCode::InternalServerError,
            ctx: Box::new(ctx_with_req),
        }));
        let analysis = analyze_error(&error);
        assert!(analysis.context_summary.contains("req-abc"));
    }
    ...
    /// 测试 defaults 模块默认重试策略详情
    #[test]
    fn test_defaults_retry_policy_details() {
        let policy = defaults::default_retry_policy();
        assert!(policy.use_exponential_backoff());
        assert_eq!(policy.max_retries(), 3);
        assert_eq!(policy.base_delay, Duration::from_secs(1));

        let delay = policy.retry_delay(0);
