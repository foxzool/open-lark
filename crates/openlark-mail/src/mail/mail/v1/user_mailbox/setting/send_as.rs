...
/// 列出可发信邮箱请求。
#[derive(Debug, Clone)]
pub struct UserMailboxSettingSendAsRequest {
    config: Arc<Config>,
    user_mailbox_id: String,
}

impl UserMailboxSettingSendAsRequest {
    /// 创建请求。
    ...
    pub async fn execute_with_options(self, option: RequestOption) -> SDKResult<serde_json::Value> {
        ...
        let path = format!(
            "/open-apis/mail/v1/user_mailboxes/{}/settings/send_as",
...
mod tests {
    ...
    fn builder_initializes() {
        ...
        let _request = UserMailboxSettingSendAsRequest::new(config);
    }
}
