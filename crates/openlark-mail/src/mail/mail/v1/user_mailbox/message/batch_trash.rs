...
/// 批量删除邮件请求。
#[derive(Debug, Clone)]
pub struct UserMailboxMessageBatchTrashRequest {
    config: Arc<Config>,
    user_mailbox_id: String,
}

impl UserMailboxMessageBatchTrashRequest {
    /// 创建请求。
    ...
    pub async fn execute_with_options(
        ...
        let path = format!(
            "/open-apis/mail/v1/user_mailboxes/{}/messages/batch_trash",
...
mod tests {
    ...
    fn builder_initializes() {
        ...
        let _request = UserMailboxMessageBatchTrashRequest::new(config);
    }
}
