...
mod tests {
    ...
    fn test_get_tasklist_tasks_request() {
        ...
        let request = GetTasklistTasksRequest::new(Arc::new(config), "tasklist_123")
            .page_size(20)
            .filter("priority = 1");

        assert_eq!(request.tasklist_guid, "tasklist_123");
        assert_eq!(request.page_size, Some(20));
        assert_eq!(request.filter, Some("priority = 1".to_string()));
    }

    #[test]
    fn test_tasklist_get_tasks_api_v2_url() {
        let endpoint = TaskApiV2::TasklistGetTasks("tasklist_123".to_string());
        assert_eq!(
            endpoint.to_url(),
            "/open-apis/task/v2/tasklists/tasklist_123/tasks"
        );
    }
}
