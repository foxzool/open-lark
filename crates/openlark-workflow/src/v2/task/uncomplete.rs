...
impl UncompleteTaskRequest {
    pub fn new(config: Arc<Config>, task_guid: String) -> Self {
        Self { config, task_guid }
    }

    /// 执行请求
    pub async fn execute(self) -> SDKResult<UncompleteTaskResponse> {
...
impl ApiResponseTrait for UncompleteTaskResponse {
...
mod tests {
    ...
    fn test_uncomplete_task_request() {
        ...
        let request = UncompleteTaskRequest::new(Arc::new(config), "task_123".to_string());
