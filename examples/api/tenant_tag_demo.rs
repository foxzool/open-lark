...
use open_lark::{
    ...
    service::tenant_tag::{
        ...
        tag_binding::{CreateTagBindingRequest, UpdateTagBindingRequest, GetTagBindingRequest},
        models::{TagType, TagStatus, UserIdType},
    },
};
...
async fn demo_tag_management(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.tenant_tag.tag.create(create_request, Some(UserIdType::OpenId), None).await {
        Ok(response) => {
            if let Some(data) = response.data {
                println!("  ✅ 标签创建成功: {} (ID: {})", data.tag.name, data.tag.tag_id);
                ...
                match client.tenant_tag.tag.patch(&tag_id, patch_request, Some(UserIdType::OpenId), None).await {
                    Ok(patch_response) => {
                ...
                // 查询标签列表
                match client.tenant_tag.tag.list(
                    Some(TagType::Chat),
                    Some(20),
                    None,
...
async fn demo_tag_binding(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
        ...
        Ok(response) => {
            if let Some(data) = response.data {
                ...
                for binding in &data.bindings {
                    println!("    - 标签ID: {}, 实体ID: {}", binding.tag_id, binding.entity_id);
                }
            }
        },
        Err(e) => {
    ...
    match client.tenant_tag.tag_binding.create(bind_request, Some(UserIdType::OpenId), None).await {
        Ok(response) => {
            if let Some(data) = response.data {
                println!("  ✅ 标签绑定成功: 成功 {} 个，失败 {} 个", 
    ...
    match client.tenant_tag.tag_binding.update(unbind_request, Some(UserIdType::OpenId), None).await {
        Ok(response) => {
            if let Some(data) = response.data {
                println!("  ✅ 标签解绑成功: 成功 {} 个，失败 {} 个", 
                    data.success_tag_ids.len(), 
                    data.failed_tag_ids.len()
                );
