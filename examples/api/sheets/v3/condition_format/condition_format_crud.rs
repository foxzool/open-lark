...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let create_req = open_lark::service::sheets::v3::condition_format::CreateConditionFormatsRequest::builder()
    ...
    match client.sheets.v3.spreadsheet_sheet.create_condition_formats(create_req, None).await {
        Ok(resp) => {
    ...
    // 获取所有条件格式
    println!("\n--- 2. 获取所有条件格式 ---");
    let get_req = open_lark::service::sheets::v3::condition_format::GetConditionFormatsRequest::builder()
        .spreadsheet_token("shtcnmBA*****yGehy8")
        .sheet_id("Sheet1")
        .build();

    match client.sheets.v3.spreadsheet_sheet.get_condition_formats(get_req, None).await {
        Ok(resp) => {
            if let Some(data) = resp.data {
                println!("✅ 获取条件格式成功!");
    ...
    if !created_cf_ids.is_empty() {
        ...
        match client.sheets.v3.spreadsheet_sheet.update_condition_formats(update_req, None).await {
            Ok(resp) => {
                if let Some(data) = resp.data {
                    println!("✅ 更新条件格式成功!");
