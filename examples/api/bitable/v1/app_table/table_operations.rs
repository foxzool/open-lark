...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let table = TableData::new("项目管理表")
        ...
        .with_fields(vec![
            ...
            TableField::single_select("优先级", vec![
                "高".to_string(), 
                "中".to_string(), 
                "低".to_string()
            ]),
            TableField::multi_select("标签", vec![
                "前端".to_string(), 
                "后端".to_string(), 
                "设计".to_string(),
                "测试".to_string()
    ...
    let create_req = open_lark::service::bitable::v1::app_table::CreateTableRequest::new(app_token, table);
    ...
    let table1 = TableData::new("用户信息表")
        .with_fields(vec![
            TableField::text("用户名"),
            TableField::text("邮箱"),
            TableField::single_select("状态", vec![
                "激活".to_string(),
                "禁用".to_string()
            ]),
        ]);

    let table2 = TableData::new("订单记录表")
        .with_fields(vec![
            TableField::text("订单号"),
            TableField::number("订单金额"),
            TableField::single_select("订单状态", vec![
                "待支付".to_string(),
                "已支付".to_string(),
                "已发货".to_string(),
                "已完成".to_string()
    ...
    let batch_create_req = open_lark::service::bitable::v1::app_table::BatchCreateTablesRequest::builder()
        .app_token(app_token)
        .add_table(table1)
        .add_table(table2)
        .build();

    let batch_table_ids = match client.bitable.v1.app_table.batch_create(batch_create_req, None).await {
        Ok(resp) => {
            if let Some(data) = resp.data {
                println!("✅ 批量创建数据表成功!");
