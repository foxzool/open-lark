...
use open_lark::{
    ...
    service::im::v2::{
        ...
        groups_bots::{BotTimeSentiveRequest, UpdateFeedCardButtonRequest, TimelyReminderRequest},
        models::{UserIdType, ButtonInfo},
    },
};
use serde_json::json;
...
async fn demo_app_feed_card(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let create_request = CreateAppFeedCardRequest {
        card_content,
        target_users: vec!["ou_example_user1".to_string(), "ou_example_user2".to_string()],
        title: Some("示例消息流卡片".to_string()),
        description: Some("这是一个演示用的消息流卡片".to_string()),
    };
    
    match client.im.v2.app_feed_card.create(create_request, None).await {
        Ok(response) => {
            if let Some(data) = response.data {
                ...
                match client.im.v2.app_feed_card.update(&card_id, update_request, None).await {
                    Ok(update_response) => {
...
async fn demo_groups_bots(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.im.v2.groups_bots.bot_time_sentive(
        UserIdType::OpenId,
        bot_request,
        None,
    ).await {
        Ok(response) => {
            if let Some(data) = response.data {
                ...
                match client.im.v2.groups_bots.update(&message_id, button_request, None).await {
                    Ok(button_response) => {
                        if let Some(button_data) = button_response.data {
                            println!("  ✅ 消息流按钮更新成功，更新了 {} 个按钮", button_data.updated_button_count);
                        }
                    },
                    Err(e) => {
                        println!("  ❌ 消息流按钮更新失败: {:?}", e);
    ...
    match client.im.v2.groups_bots.patch(
        UserIdType::OpenId,
        reminder_request,
        None,
    ).await {
