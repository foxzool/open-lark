...
async fn main() -> SDKResult<()> {
    ...
    match image_service.write_image(basic_request, None).await {
        Ok(response) => {
            ...
            if let Some(data) = &response.data {
                ...
                println!("尺寸: {}x{}", data.actual_size.width, data.actual_size.height);
    ...
    match image_service.write_image(builder_request, None).await {
        ...
        Err(error) => {
            println!("❌ Builder模式图片写入失败: {}", error.user_friendly_message());
        }
    }
    ...
    match image_service.write_images_batch("shtcnmBA*****yGehy8", batch_requests, None).await {
        Ok(responses) => {
            println!("✅ 批量图片写入完成!");
                ...
                match response {
                    Ok(resp) => {
                        if let Some(data) = &resp.data {
                            println!("  图片{}: ID={}, 位置={}", index + 1, data.image_id, data.position.anchor_cell);
                        }
                    }
                    Err(error) => {
                        println!("  图片{} 写入失败: {}", index + 1, error.user_friendly_message());
                    }
                }
            }
        }
    ...
    match image_service.write_images_batch("shtcnmBA*****yGehy8", dashboard_requests, None).await {
        Ok(responses) => {
            println!("✅ 仪表板布局创建完成!");
    ...
    match image_service.write_image(dynamic_request, None).await {
        Ok(response) => {
            ...
            if let Some(data) = &response.data {
                ...
                println!("图片实际尺寸: {}x{}", data.actual_size.width, data.actual_size.height);
    ...
    match image_service.write_image(initial_request, None).await {
        Ok(response) => {
            if let Some(data) = &response.data {
                ...
                // 获取图片信息
                match image_service.get_image_info("shtcnmBA*****yGehy8", &image_id, None).await {
                    Ok(info_response) => {
                        if let Some(info) = &info_response.data {
                            println!("图片信息获取成功:");
                            println!("  位置: {}", info.position.anchor_cell);
                            println!("  尺寸: {}x{}", info.actual_size.width, info.actual_size.height);
                ...
                match image_service.update_image(
                    "shtcnmBA*****yGehy8",
                    &image_id,
                    new_position,
                    new_size,
                    ...
                    Ok(update_response) => {
                        ...
                        if let Some(data) = &update_response.data {
                            ...
                            println!("新尺寸: {}x{}", data.actual_size.width, data.actual_size.height);
                        }
                    }
                    Err(error) => {
                ...
                // 删除图片
                match image_service.delete_image("shtcnmBA*****yGehy8", &image_id, None).await {
                    Ok(_) => {
                        println!("✅ 图片删除成功!");
                    }
                    Err(error) => {
    ...
    match invalid_url_request {
        Ok(request) => {
            match image_service.write_image(request, None).await {
                Ok(_) => {
                    println!("意外成功，应该失败");
                }
                Err(error) => {
        ...
        Err(error) => {
            println!("✅ 正确在构建阶段捕获URL错误: {}", error.user_friendly_message());
        }
    }
    ...
    match invalid_cell_request {
        Ok(request) => {
            match image_service.write_image(request, None).await {
                ...
                Err(error) => {
                    println!("✅ 正确捕获无效单元格错误: {}", error.user_friendly_message());
                }
            }
        }
        Err(error) => {
            println!("✅ 正确在构建阶段捕获单元格错误: {}", error.user_friendly_message());
        }
    }
    ...
    match image_service.write_images_batch("shtcnmBA*****yGehy8", large_batch_requests, None).await {
        Ok(responses) => {
