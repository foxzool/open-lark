...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("--- 获取订阅状态 ---");
    let get_request =
        open_lark::service::assistant::v1::subscription::GetSubscriptionRequest::builder()
    ...
    println!("\n--- 创建基础订阅 ---");
    let create_request =
        open_lark::service::assistant::v1::subscription::CreateSubscriptionRequest::builder()
            .file_token(file_token)
            .as_doc()
            .basic_subscription()
            .build();
    ...
    let activate_request =
        open_lark::service::assistant::v1::subscription::PatchSubscriptionRequest::builder()
            .file_token(file_token)
    ...
    match client
        ...
        .v1
        .subscription
        .patch(quick_activate_request, None)
        .await
    {
        Ok(response) => {
            println!("快速激活成功:");
    ...
    match client
        ...
        .v1
        .subscription
        .patch(eco_activate_request, None)
        .await
    {
        Ok(response) => {
            println!("节能模式激活成功:");
    ...
    match client
        ...
        .v1
        .subscription
        .quick_subscribe_doc("doc_new_xxxxx", None)
        .await
    {
        Ok(response) => {
    ...
    match client
        ...
        .v1
        .subscription
        .is_subscribed(file_token, file_type.clone(), None)
        .await
    {
        Ok(is_subscribed) => {
            println!(
    ...
    match client
        ...
        .v1
        .subscription
        .quick_activate(file_token, file_type.clone(), None)
        .await
    {
        Ok(response) => {
    ...
    let files_to_subscribe = vec![
        (
            "doc1_xxxxx".to_string(),
            open_lark::service::assistant::v1::subscription::FileType::Doc,
        ),
        (
            "sheet1_xxxxx".to_string(),
            open_lark::service::assistant::v1::subscription::FileType::Sheet,
        ),
        (
            "wiki1_xxxxx".to_string(),
            open_lark::service::assistant::v1::subscription::FileType::Wiki,
        ),
    ];
    ...
    let batch_results = client
        ...
        .v1
        .subscription
        .batch_subscribe(files_to_subscribe, None)
        .await;
    ...
    for file_type in [&doc_type, &bitable_type, &sheet_type, &wiki_type] {
        println!(
            ...
            if file_type.supports_assistant() {
                "是"
            } else {
                "否"
            }
    ...
    // 13. 订阅优先级和配置展示
    println!("\n--- 订阅优先级和配置展示 ---");
    use open_lark::service::assistant::v1::subscription::{
    ...
    for (name, config) in configs {
        ...
        println!(
            "  通知频率: {:.1} 小时",
            config.get_notification_interval_hours()
        );
