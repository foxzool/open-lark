...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.permission.get_permission_public_v2(&request, None).await {
        Ok(response) => {
            println!("获取权限设置成功 (v2)");
            ...
            println!("- 组织外分享: {}", if settings.allows_external_share() { "允许" } else { "不允许" });
            ...
            println!("- 高级配置: {}", if response.data.has_advanced_config() { "已启用" } else { "未启用" });
        }
        Err(e) => {
            eprintln!("获取权限设置失败: {:?}", e);
        }
    ...
    match client.permission.get_permission_public_v2(&sheet_request, None).await {
        Ok(response) => {
            println!("电子表格权限设置:");
            let settings = response.data.public_settings();
    ...
    match client.permission.get_permission_public_v2(&bitable_request, None).await {
        Ok(response) => {
            println!("多维表格权限设置:");
            let settings = response.data.public_settings();
            ...
            // 检查安全风险
            let recommendations = response.data.security_recommendations();
            let risk_count = recommendations.iter()
