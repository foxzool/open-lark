...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.permission.patch_permission_public_v2(&request, None).await {
        Ok(response) => {
            println!("设置企业级安全模式成功");
    ...
    match client.permission.patch_permission_public_v2(&collaboration_request, None).await {
        Ok(response) => {
    ...
    match client.permission.patch_permission_public_v2(&public_request, None).await {
        Ok(response) => {
            ...
            // 安全警告检查
            let recommendations = response.data.operation_recommendations();
            let warnings = recommendations.iter()
    ...
    match client.permission.patch_permission_public_v2(&open_request, None).await {
        Ok(response) => {
    ...
    match client.permission.patch_permission_public_v2(&custom_request, None).await {
        Ok(response) => {
            println!("自定义高级权限设置成功");
    ...
    match client.permission.patch_permission_public_v2(&permanent_request, None).await {
        Ok(response) => {
            println!("永久有效权限设置成功");
