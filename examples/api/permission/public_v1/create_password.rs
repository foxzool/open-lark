...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.permission.create_password(&random_request, None).await {
        Ok(response) => {
            ...
            println!("密码类型: {}", if password_info.is_numeric_password() { "纯数字" } else { "混合字符" });
            println!("密码长度: {}", password_info.password_length());
            
            println!("创建状态: {}", if response.data.is_created() { "成功" } else { "失败" });
        }
        Err(e) => {
            eprintln!("设置随机密码失败: {:?}", e);
        }
    ...
    match client.permission.create_password(&wiki_request, None).await {
        Ok(response) => {
            ...
            if password_info.password_strength() == "很强" || password_info.password_strength() == "强" {
                println!("✓ 密码强度良好");
            }
        }
        Err(e) => {
