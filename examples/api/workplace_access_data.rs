...
use open_lark::{
    prelude::*,
    service::workplace::workplace_access_data::*,
};
use std::env;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        .await
    {
        Ok(response) => {
            if let Some(search_data) = response.data {
                ...
                if let Some(has_more) = search_data.access_data.has_more {
                    if has_more {
    ...
    {
        Ok(response) => {
            if let Some(custom_data) = response.data {
    ...
    {
        Ok(response) => {
            if let Some(widget_data) = response.data {
