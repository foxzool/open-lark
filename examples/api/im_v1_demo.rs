...
use open_lark::{
    ...
    service::im::v1::{
        ...
        models::{UserIdType, ReceiveIdType},
...
async fn demo_message_reactions(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 添加表情回复
    match client.im.v1.message_reaction.create(
        message_id,
        emoji_type,
    ...
    // 获取表情回复列表
    match client.im.v1.message_reaction.list(
        message_id,
        Some(UserIdType::OpenId),
        Some(20),
        None,
...
async fn demo_image_upload(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 上传图片
    match client.im.v1.image.create(
        "png",
        image_data,
        None,
    ).await {
                ...
                match client.im.v1.image.get(&data.image_key, None).await {
                    Ok(download_response) => {
                        if let Some(download_data) = download_response.data {
                            println!("  📥 图片下载成功，大小: {} bytes", download_data.data.len());
                        }
                    },
                    Err(e) => {
                        println!("  ❌ 图片下载失败: {:?}", e);
...
async fn demo_batch_messages(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 批量发送消息
    match client.im.v1.batch_message.send(
        ...
        Ok(response) => {
            if let Some(data) = response.data {
                ...
                // 查询批量消息进度
                match client.im.v1.batch_message.get_progress(&data.batch_message_id, None).await {
                    Ok(progress_response) => {
                        if let Some(progress_data) = progress_response.data {
                            println!("  📊 批量消息进度: 总数{}, 成功{}, 失败{}", 
...
async fn demo_message_cards(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 更新消息卡片
    match client.im.v1.message_card.patch(message_id, request, None).await {
        Ok(response) => {
...
async fn demo_urgent_messages(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 发送应用内加急
    match client.im.v1.buzz_messages.urgent_app(
        message_id,
        UserIdType::OpenId,
        request,
        None,
        ...
        Ok(response) => {
            if let Some(data) = response.data {
                println!("  ✅ 应用内加急发送成功，无效用户数: {}", data.invalid_user_id_list.len());
            }
        },
        Err(e) => {
            println!("  ❌ 应用内加急发送失败: {:?}", e);
...
async fn demo_url_preview(client: &LarkClient) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 批量更新URL预览
    match client.im.v1.url_preview.batch_update(message_id, request, None).await {
        Ok(response) => {
