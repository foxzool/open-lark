...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.board.list_nodes(&request, None).await {
        Ok(response) => {
            ...
            // 绘图元素分析
            println!("\n--- 绘图元素分析 ---");
            let drawing_nodes: Vec<_> = nodes_data.items.iter()
            ...
            // 内容元素分析
            println!("\n--- 内容元素分析 ---");
            let content_type_nodes: Vec<_> = nodes_data.items.iter()
            ...
            if !content_type_nodes.is_empty() {
                ...
                if text_nodes > 0 {
                    println!("- 文本节点: {}个，总字符数: {}", text_nodes, total_text_length);
                }
                if image_nodes > 0 {
            ...
            // 容器元素分析
            let container_nodes: Vec<_> = nodes_data.items.iter()
