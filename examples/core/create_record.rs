...
use open_lark::service::cloud_docs::bitable::v1::app_table_record::create::*;
        ...
        .await
    {
        Ok(response) => {
            if let Some(data) = &response.data {
                println!("✅ 简单记录创建成功!");
                println!("   记录ID: {}", data.record.record_id);
                println!("   创建时间: {}", data.record.created_time);
...
async fn create_complex_record(
    ...
    // 日期字段（毫秒时间戳）
    fields.insert("截止日期".to_string(), json!(chrono::Utc::now().timestamp_millis()));
    ...
    // 链接字段
    fields.insert("相关链接".to_string(), json!({
        "text": "飞书开放平台",
        "link": "https://open.feishu.cn"
    }));
    
    // JSON对象字段
    fields.insert("元数据".to_string(), json!({
        "source": "rust_sdk",
        "version": "1.0.0",
