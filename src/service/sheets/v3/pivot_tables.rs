...
mod tests {
    ...
    fn test_pivot_table_config_creation() {
        ...
        let config = PivotTableConfig::new("sheet123".to_string(), source_range, position)
            .add_row_field(row_field)
            .add_column_field(column_field)
            .add_value_field(value_field)
            .layout(layout);
    ...
    fn test_pivot_table_config_validation() {
        ...
        // 测试有效配置
        let valid_config = PivotTableConfig::new("sheet123".to_string(), source_range, position)
            .add_row_field(PivotField::new("部门".to_string(), 0))
            .add_value_field(value_field);
        assert!(valid_config.validate().is_ok());

        // 测试空工作表ID
        let invalid_config = PivotTableConfig::new(
            "".to_string(),
        ...
        // 测试无值字段
        let invalid_config2 = PivotTableConfig::new(
            "sheet123".to_string(),
            Range::new("A1".to_string(), "D100".to_string()),
            PivotTablePosition::new("sheet123".to_string(), 1, 1),
        ).add_row_field(PivotField::new("部门".to_string(), 0));
        ...
        // 测试无行列字段
        let invalid_config3 = PivotTableConfig::new(
            "sheet123".to_string(),
            Range::new("A1".to_string(), "D100".to_string()),
            PivotTablePosition::new("sheet123".to_string(), 1, 1),
        ).add_value_field(ValueField::new("销售额".to_string(), 2, SummaryFunction::Sum));
