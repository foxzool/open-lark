...
//! ## 功能描述
//!
//! 此接口用于部分更新指定日程事件的信息，支持更新事件标题、描述、时间、
//! 地点、参与者等信息的任意组合，无需提供完整的事件数据。
//!
//! ## API端点
//!
...
impl PatchCalendarEventRequest {
    ...
    /// # 参数
    /// - `color`: 日程颜色
    pub fn set_color(&mut self, color: i32) -> &mut Self {
...
/// 包含更新后的日程事件信息。
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PatchCalendarEventResponse {
    /// 更新后的日程事件信息
...
impl ApiResponseTrait for PatchCalendarEventResponse {
...
/// 提供流式API来构建更新日程事件的请求。
#[derive(Debug, Clone)]
pub struct PatchCalendarEventBuilder {
    request: PatchCalendarEventRequest,
}

impl PatchCalendarEventBuilder {
    /// 创建更新日程事件构建器
    ///
    /// # 参数
    /// - `calendar_id`: 日历ID
    ...
    ///
    /// ```rust
    /// use open_lark::service::calendar::v4::calendar_event::PatchCalendarEventBuilder;
    ///
    /// let builder = PatchCalendarEventBuilder::new("calendar_123", "event_456");
    /// ```
    pub fn new(calendar_id: impl Into<String>, event_id: impl Into<String>) -> Self {
        Self {
            request: PatchCalendarEventRequest::new(calendar_id, event_id),
        }
    }
    ...
    ///
    /// # 返回
    /// - `Ok(PatchCalendarEventRequest)`: 构建成功的请求对象
    /// - `Err(String)`: 构建失败，返回错误信息
    pub fn build(self) -> Result<PatchCalendarEventRequest, String> {
    ...
    ///
    /// # 返回
    /// - `Ok(PatchCalendarEventResponse)`: 更新成功，返回日程事件信息
    ...
    /// let response = PatchCalendarEventBuilder::new("calendar_123", "event_456")
    ///     .summary("更新的团队会议")
...
impl Default for PatchCalendarEventBuilder {
    fn default() -> Self {
        Self {
            request: PatchCalendarEventRequest {
...
mod tests {
    ...
    #[test]
    fn test_patch_calendar_event_request_creation() {
        let request = PatchCalendarEventRequest::new("calendar_123", "event_456");
    ...
    #[test]
    fn test_set_summary() {
        let mut request = PatchCalendarEventRequest::new("calendar_123", "event_456");
        request.set_summary("更新的会议");
        assert_eq!(request.summary, Some("更新的会议".to_string()));
    }

    #[test]
    fn test_set_description() {
        let mut request = PatchCalendarEventRequest::new("calendar_123", "event_456");
        request.set_description("更新的描述");
    ...
    fn test_set_color() {
        let mut request = PatchCalendarEventRequest::new("calendar_123", "event_456");
        request.set_color(3);
        assert_eq!(request.color, Some(3));
    }
    ...
    fn test_request_validation_empty_calendar_id() {
        let request = PatchCalendarEventRequest::new("", "event_456");
    ...
    fn test_request_validation_empty_event_id() {
        let request = PatchCalendarEventRequest::new("calendar_123", "");
        let result = request.validate();
        assert!(result.is_err());
    ...
    #[test]
    fn test_patch_calendar_event_builder_creation() {
        let builder = PatchCalendarEventBuilder::new("calendar_123", "event_456");
    ...
    fn test_builder_build_success() {
        let result = PatchCalendarEventBuilder::new("calendar_123", "event_456")
            .summary("更新的会议")
            .user_id_type("open_id")
            .build();
        assert!(result.is_ok());
    ...
    fn test_builder_build_failure_empty_calendar_id() {
        let result = PatchCalendarEventBuilder::new("", "event_456").build();
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "日历ID不能为空");
    }
    ...
    fn test_builder_build_failure_empty_event_id() {
        let result = PatchCalendarEventBuilder::new("calendar_123", "").build();
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "日程ID不能为空");
    }
    ...
    fn test_builder_default() {
        let builder = PatchCalendarEventBuilder::default();
    ...
    fn test_serialization_request() {
        let request = PatchCalendarEventRequest {
            calendar_id: "calendar_123".to_string(),
            event_id: "event_456".to_string(),
            summary: Some("更新的会议".to_string()),
        ...
        assert!(json.contains("更新的会议"));
        assert!(json.contains("更新的描述"));
    ...
    fn test_default_response() {
        let response = PatchCalendarEventResponse::default();
    ...
    fn test_fluent_api_chain() {
        ...
        assert_eq!(request.summary, Some("更新的会议".to_string()));
        assert_eq!(request.description, Some("更新的描述".to_string()));
        assert_eq!(request.color, Some(2));
        assert_eq!(request.user_id_type, Some("user_id".to_string()));
    }
    ...
    fn test_builder_fluent_with_all_options() {
        ...
        assert_eq!(builder.request.summary, Some("更新的会议".to_string()));
        assert_eq!(builder.request.description, Some("更新的描述".to_string()));
        assert_eq!(builder.request.is_all_day, Some(false));
        assert_eq!(builder.request.color, Some(1));
    ...
    fn test_validation_edge_cases() {
        ...
        let request = PatchCalendarEventRequest::new(&long_id, &long_id);
        assert!(request.validate().is_ok());

        // Test with Unicode characters
        let unicode_request = PatchCalendarEventRequest::new("日历_123", "事件_456");
