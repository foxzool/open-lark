...
```mermaid
graph TD
    A["(根) open-lark"] --> B["examples"];
    A --> C["crates"];
    A --> D["tools"];
    A --> E["docs"];

    C --> C1["openlark-core<br/>核心基础设施"];
    C --> C2["openlark-client<br/>高级客户端封装"];
    C --> C3["openlark-protocol<br/>WebSocket协议定义"];
    C --> C4["openlark-communication<br/>通讯服务模块"];
    C --> C5["openlark-hr<br/>人力资源服务"];
    C --> C6["openlark-auth<br/>认证服务"];
    C --> C7["openlark-ai<br/>AI服务"];
    C --> C8["openlark-docs<br/>云文档服务"];
    C --> C9["openlark-application<br/>应用服务"];
    C --> C10["其他服务模块..."];
...
| 模块路径 | 职责描述 | 主要功能 | 覆盖率 | 状态 |
|---------|---------|---------|--------|------|
| `crates/openlark-core` | 核心基础设施 | HTTP客户端、配置、错误处理、令牌管理、WebSocket支持 | 🟢 85% | ✅ 活跃 |
| `crates/openlark-client` | 客户端库 | LarkClient、服务注册、异步接口、构建器模式 | 🟡 60% | ✅ 活跃 |
| `crates/openlark-protocol` | 协议定义 | WebSocket protobuf定义、消息协议 | 🟢 90% | ✅ 活跃 |
| `crates/openlark-communication` | 通讯服务 | IM消息、联系人管理、群组、消息卡片 | 🟡 70% | ✅ 活跃 |
| `crates/openlark-hr` | 人力资源 | 考勤、CoreHR、EHR、招聘、员工管理 | 🟡 65% | ✅ 活跃 |
| `crates/openlark-auth` | 认证服务 | 令牌管理、身份验证、权限控制 | 🟢 80% | ✅ 活跃 |
| `crates/openlark-ai` | AI服务 | 智能助手、AI功能、自然语言处理 | 🟡 55% | ✅ 活跃 |
| `crates/openlark-docs` | 云文档服务 | 文档、表格、知识库、Wiki API | 🟡 50% | ✅ 活跃 |
| `crates/openlark-application` | 应用服务 | 应用管理、配置、部署 | 🔴 30% | 🚧 开发中 |
| `examples` | 示例代码 | API使用示例、业务场景演示、最佳实践 | 🟢 95% | ✅ 活跃 |
| `tools` | 开发工具 | API一致性检查、代码质量工具、构建脚本 | 🟡 40% | ✅ 活跃 |

### 服务器模块详细映射

| 服务分类 | Crate模块 | 覆盖的API | 测试覆盖度 |
|---------|-----------|-----------|------------|
| **通讯协作** | openlark-communication | IM、消息、联系人、群组 | 🟡 70% |
| **人力资源** | openlark-hr | 考勤、CoreHR、EHR、招聘 | 🟡 65% |
| **文档管理** | openlark-docs | 云文档、表格、Wiki、知识库 | 🟡 50% |
| **智能服务** | openlark-ai | AI助手、智能分析、自然语言处理 | 🟡 55% |
| **身份认证** | openlark-auth | 令牌、权限、身份验证 | 🟢 80% |
